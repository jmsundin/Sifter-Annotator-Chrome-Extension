...
function stripMobilizer(url) {
  ...
  if (i == 0) return decodeURIComponent(url.substring(VIEWTEXT_PREFIX.length));
...
function isSignin(url) {
  ...
  return (
    lowUrl.indexOf("accounts.google") !== -1 &&
    (lowUrl.indexOf("signin") !== -1 || lowUrl.indexOf("login") !== -1)
  );
}
...
function yawas_getElement(xml, elementname) {
  ...
  if (e.length == 0) e = xml.getElementsByTagName("smh:" + elementname);
  if (e == null) console.log("yawas_getElement null for: " + elementname);
  return e;
}
...
async function yawas_getAnnotations(webUrl, cb) {
  if (saveLocally) return yawas_getAnnotations_chrome_storage(webUrl);
  ...
  var url =
    "https://www.google.com/bookmarks/find?output=rss&q=" + escape(webUrl);
    ...
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        ...
        if (xhr.responseURL === url) {
          ...
          } else cb({ error: "logged out", signedout: true });
...
function sendMessageActiveTab(json) {
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    if (tabs && tabs.length > 0) {
      ...
      port.onDisconnect = function (err) {
...
function formatAnnotation(text) {
  var newtext = text
    .replace(/&ldquo;/gi, leftMark)
    .replace(/&rdquo;/gi, rightMark);
...
function yawas_storeHighlight(
  ...
  if (!webLabels) webLabels = "";
  webAnnotation = formatAnnotation(webAnnotation);
  if (occurence === 0) {
    if (pagenumber)
      webAnnotation +=
        leftMark + highlight + "@" + occurence + "," + pagenumber;
    else webAnnotation += leftMark + highlight;
  } else {
    webAnnotation += leftMark + highlight + "@" + occurence;
    if (pagenumber) webAnnotation += "," + pagenumber;
  }
  if (couleur != "yellow") webAnnotation += "#" + couleur;
  webAnnotation += rightMark + " ";
  let pureLen = webAnnotation.length;
  if (!saveChromeBookmarks && webAnnotation.length > 2048) {
    console.log(
      "too long so compacting annotations",
      qurl,
      webAnnotation.length
    );
...
async function yawas_storeHighlightsNow(
  webUrl,
  title,
  labels,
  annotations,
  ...
  if (saveChromeBookmarks) {
    ...
    var folderName = new Date().toLocaleDateString("en-US", {
      year: "numeric",
      month: "short",
    });
    //console.log('obj=',obj);
...
function yawas_copyclipboard(url, title) {
  url = purifyURL(url);
  if (title.trim().length == 0) title = "no title";
  var webAnnotation = cachedAnnotations[url];
  var highlights = annotationToArray(webAnnotation);
  var body = title + "\n" + url + "\n";
  for (var i = 0; i < highlights.length; i++) {
    if (highlights[i].comment) body += highlights[i].comment + " ";
...
function updateHighlight(
  ...
  if (j == -1) j = qurl.lastIndexOf("/index.html");
  if (j != -1) qurl = qurl.substring(0, j + 1);
  ...
  if (idx == -1) {
    ...
    if (newcolor === "note") highlights[idx].comment = comment;
    else highlights[idx].color = newcolor;
...
function delHighlightNow(fragment, occurence, url, title, pagenumber, cb) {
    ...
    if (same && highlights[i].n == occurence) {
      if (idx != -1) {
        console.log(
          "[" +
            fragment +
            "] found in more than one highlight, please select more text to identify which highlight to delete"
        );
        return cb({ error: "found in more than one highlight" });
      } else idx = i;
    }
  ...
  if (idx == -1) {
    console.log(
      "delHighlightNow error: Highlight not found [" + fragment + "]"
    );
...
function isPDF(href) {
  if (href.indexOf("pdf_viewer.html") === -1) return href;
  let comps = href.split("?file=");
  if (comps.length > 1) return decodeURIComponent(comps[1]);
  else return href;
}
  ...
  if (info.menuItemId === "delete")
    ...
    chrome.tabs.create({
      url: chrome.runtime.getURL(
        "localedit.html?url=" + encodeURIComponent(purifyURL(possiblePDFUrl))
      ),
    });
  } else {
